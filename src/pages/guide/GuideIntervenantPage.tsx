// src/pages/guide/GuideIntervenantPage.tsx — Guide complet pour les intervenants
import { useState, useMemo } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import { BookOpen, Search, Bell, ChevronRight, Rocket, Check, HelpCircle } from 'lucide-react'
import { ALL_INTERVENANT_SECTIONS } from '@/lib/data/guide-intervenant'
import { useGuideNews, useCompletedSlugs } from '@/lib/hooks/guide'
...
export default function GuideIntervenantPage() {
  ...
  const [activeSlug, setActiveSlug] = useState(sectionParam || ALL_INTERVENANT_SECTIONS[0].slug)
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const [newsExpanded, setNewsExpanded] = useState(false)

  const activeSection = ALL_INTERVENANT_SECTIONS.find(s => s.slug === activeSlug) ?? ALL_INTERVENANT_SECTIONS[0]

  const filteredSections = useMemo(() => {
    if (!search.trim()) return ALL_INTERVENANT_SECTIONS
    const q = ns(search)
    return ALL_INTERVENANT_SECTIONS.filter(s =>
  ...
  function selectSection(slug: string) {
    ...
    nav(`/guide/intervenant/${slug}`, { replace: true })
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p className="page-subtitle">Tout ce dont vous avez besoin sur le terrain.</p>
        </div>
      ...
      <GuideProgress sections={ALL_INTERVENANT_SECTIONS} />
