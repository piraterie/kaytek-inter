...
function ConnectionCard({
  ...
  return (
    <div className="card" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {isConnected && formatDateTime(info?.connected_at) && (
        <div style={{ fontSize: 12, color: 'var(--t2, #6b7280)' }}>Connecté le {formatDateTime(info?.connected_at)}</div>
      )}

      {isConnected && formatDateTime(info?.last_synced_at) && (
        <div style={{ fontSize: 12, color: 'var(--t2, #6b7280)' }}>Dernière synchronisation : {formatDateTime(info?.last_synced_at)}</div>
      )}
...
export default function IntegrationsGooglePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useGoogleOAuthStatus()
  const connectMut = useConnectGoogle()
  const disconnectMut = useDisconnectGoogle()
  const handledCallback = useRef(false)
  const [confirmDisconnect, setConfirmDisconnect] = useState<GoogleProvider | null>(null)
  ...
  function requestDisconnect(provider: GoogleProvider) {
    setConfirmDisconnect(provider)
  }

  function confirmDisconnectNow() {
  ...
  const confirmDisconnectLabel = confirmDisconnect === 'google_ads' ? 'Google Ads' : 'Google Business Profile'

  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
      <div className="params-section-label">Connexion Google</div>
      <p style={{ fontSize: 13, color: 'var(--t2, #6b7280)', marginBottom: 16 }}>
        Connectez les comptes Google de votre organisation pour Google Ads et Google Business Profile
        (établissements, avis). Réservé aux administrateurs. Les jetons d'accès Google restent chiffrés côté serveur
        et ne sont jamais transmis au navigateur.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="card" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ fontSize: 13, color: 'var(--rdTx, #dc2626)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            Impossible de récupérer le statut des connexions Google
            {error instanceof Error && error.message ? ` (${error.message})` : ''}.
          </div>
          <button className="btn-secondary" style={{ alignSelf: 'flex-start' }} onClick={() => refetch()}>
      ...
      {confirmDisconnect && (
        <ConfirmModal
          message={`Déconnecter ${confirmDisconnectLabel} ? L'accès aux données de ce compte Google sera coupé jusqu'à une nouvelle connexion.`}
