  ...
  function handleEmail(f: Facture) {
    ...
    ;(async () => {
      ...
      try {
        ...
        const paiementImmediat = !!f.date_echeance && f.date_echeance === f.date_emission
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:620px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
          ...
          <div style="padding:32px 40px;"><p style="margin:0 0 16px;font-size:15px;color:#374151;">Bonjour <strong>${f.client?.prenom || ''} ${f.client?.nom || ''}</strong>,</p><p style="margin:0 0 16px;font-size:15px;color:#374151;">Veuillez trouver ci-joint votre facture pour nos prestations de serrurerie. ${estPayee ? 'Merci pour votre paiement.' : paiementImmediat ? 'Merci de procéder au règlement dès réception (paiement immédiat).' : 'Merci de procéder au règlement avant la date d\'échéance indiquée.'}</p>
          <div style="background:#f8fafc;border-left:4px solid ${accentColor};border-radius:6px;padding:20px 24px;margin:24px 0;"><div style="font-size:12px;color:#6b7280;text-transform:uppercase;margin-bottom:6px;">Montant total TTC</div><div style="font-size:28px;font-weight:700;color:${theme.primary};">${(f.montant_ttc||0).toLocaleString('fr-FR',{style:'currency',currency:'EUR'})}</div>${!estPayee && f.date_echeance ? (paiementImmediat ? `<div style="font-size:12px;color:${theme.accent};margin-top:8px;">💳 Paiement immédiat — à régler dès réception</div>` : `<div style="font-size:12px;color:${theme.accent};margin-top:8px;">⏳ À régler avant le <strong>${new Date(f.date_echeance).toLocaleDateString('fr-FR')}</strong></div>`) : ''}${estPayee && f.date_paiement ? `<div style="font-size:12px;color:#16a34a;margin-top:8px;">✓ Réglée le ${new Date(f.date_paiement).toLocaleDateString('fr-FR')}</div>` : ''}</div>
