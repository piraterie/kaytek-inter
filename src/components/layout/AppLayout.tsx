...
const NI_STYLE: React.CSSProperties = { width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 9px', borderRadius: 7, cursor: 'pointer', border: 'none', background: 'transparent', color: 'rgba(255,255,255,.52)', marginBottom: 1, transition: 'all .12s', whiteSpace: 'nowrap', overflow: 'hidden', fontFamily: 'inherit', minHeight: 44 }
    ...
    <div style={{ display: 'flex', height: '100dvh', overflow: 'hidden' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '16px 13px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid rgba(255,255,255,.07)', flexShrink: 0 }}>
          <div style={{ width: 28, height: 28, background: '#2563eb', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>🔐</div>
          {sidebarOpen && <div style={{ overflow: 'hidden' }}>
            <div style={{ color: '#fff', fontSize: 14, fontWeight: 600, whiteSpace: 'nowrap' }}>Kaytek Inter</div>
            <div style={{ color: 'rgba(255,255,255,.45)', fontSize: 11 }}>Serrurerie · Vitrerie</div>
          </div>}
        </div>
        {/* Nav */}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden', background: 'var(--bg)' }}>
        {/* Topbar */}
        <header style={{ height: 52, background: 'var(--s0)', borderBottom: '1px solid var(--b0)', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 10, flexShrink: 0 }}>
          <button className="btn-icon" onClick={toggleSidebar} aria-label="Menu" style={{ fontSize: 18, flexShrink: 0 }}>☰</button>
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--t0)' }}>{currentPage}</span>
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {notifOpen && (
                <div style={{ position: 'fixed', top: 52, right: isMobile ? 8 : 16, width: isMobile ? 'calc(100vw - 16px)' : 360, maxHeight: 500, background: 'var(--s0)', border: '1px solid var(--b1)', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,.2)', zIndex: 500, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
        ...
        {/* Content */}
        <main style={{ flex: 1, overflowY: 'auto', padding: isMobile ? 16 : 18, scrollbarWidth: 'thin', scrollbarColor: 'var(--s3) transparent' }}>
          <Outlet />
        </main>
      </div>
