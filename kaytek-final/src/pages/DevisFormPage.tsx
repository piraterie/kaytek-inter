...
export default function DevisFormPage() {
  ...
  const tot = lignes.reduce((a, l) => ({
    ht: a.ht + l.total_ht,
    ttc: a.ttc + l.total_ttc,
    tva: a.tva + Math.round(l.total_ht * l.tva_pct / 100 * 100) / 100
  }), { ht: 0, ttc: 0, tva: 0 })
  const remise_ht = Math.round(tot.ht * (form.remise_pct / 100) * 100) / 100
  const remise = Math.round(tot.ttc * (form.remise_pct / 100) * 100) / 100
  const totalFinal = Math.round((tot.ttc - remise) * 100) / 100
  const tva = Math.max(0, Math.round((tot.tva * (1 - form.remise_pct / 100)) * 100) / 100)
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      {!params && (
        <div style={{ background:'var(--amBg)', border:'1px solid var(--amBd)', borderRadius:'var(--r2)', padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
          <span style={{ fontSize:18 }}>⚠</span>
          <div>
            <div style={{ fontWeight:600, color:'var(--amTx)', fontSize:13 }}>Parametres entreprise manquants</div>
            <div style={{ fontSize:12, color:'var(--amTx)' }}>La generation PDF ne fonctionnera pas. <a href="/parametres" style={{ color:'var(--am)', fontWeight:600 }}>Remplir maintenant →</a></div>
          </div>
        </div>
      )}
      ...
      <div className="card card-body mb-4">
        ...
        {showSig && !existing?.signature_url && (
          <div>
            <div style={{ background: 'var(--s1)', border: '2px dashed var(--bl)', borderRadius: 10, padding: 6, position: 'relative' }}>
              <p style={{ fontSize: 11, color: 'var(--t3)', textAlign: 'center', paddingTop: 4, marginBottom: 4 }}>✍ Signez dans la zone ci-dessous</p>
              <canvas ref={canvasRef} width={800} height={200}
                style={{ display: 'block', width: '100%', height: 160, touchAction: 'none', cursor: 'crosshair', borderRadius: 8, background: 'var(--s0)' }} />
            </div>
            <div className="flex gap-2 mt-3" style={{ flexWrap: 'wrap' }}>
              <button className="btn btn-secondary" onClick={() => sigPad?.clear()} style={{ flex: 1 }}>🗑 Effacer</button>
              {isEdit
                ? <button className="btn btn-primary" onClick={handleSign} style={{ flex: 2 }}>✓ Valider la signature</button>
                : <button className="btn btn-secondary" onClick={() => save('brouillon')} style={{ flex: 2, color: 'var(--amTx)', borderColor: 'var(--amBd)' }}>Sauvegarder d abord →</button>
              }
            </div>
          </div>
        )}
