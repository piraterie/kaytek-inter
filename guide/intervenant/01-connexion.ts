// guide/intervenant/01-connexion.ts — Connexion intervenant
...
test('guide intervenant — connexion', async ({ page }) => {
  ...
  if (onLoginPage) {
    ...
    await showTitle(page, 'Connexion', 'Guide Intervenant — Kaytek Inter')
  ...
  await injectVisualLayer(page)
  if (!onLoginPage) await showTitle(page, 'Connexion', 'Guide Intervenant — Kaytek Inter')

  await showStep(page, 'Tableau de bord')
  await showTip(page, '🏠 Vous êtes connecté — voici votre espace intervenant')
